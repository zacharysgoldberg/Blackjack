    ...
    while True:
        ...
        # If player score is < 21 / if split, both hands must be < 21 to hit or stand
            ...
            # If player stands, and player score is <= 21 / if split, at least one hand is <= 21, And dealer hand is less than player hand(s); win
                  ...
                  ('Score' in new_game.user and new_game.dealer['Score'] > 21) or ('Score' not in new_game.user and new_game.dealer['Score'] <= new_game.user['Score 1']) or
                    ('Score' not in new_game.user and new_game.dealer['Score'] <= new_game.user['Score 2'])):
                win_lose.win()
                win_lose.total_wins += 1
                break
